...
class DataCollatorForLanguageModeling:
    ...
    def mask_tokens(self, inputs: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        max_mlm_tokens = inputs.size(1) * self.mlm_probability
        mlm_length_distribution = np.array([0.2 * (1 - 0.2) ** (i - 1) for i in range(1, 11)])
        mlm_length_distribution /= mlm_length_distribution.sum()
        for i in range(inputs.size(0)):
            num_mlm_tokens = 0
            while num_mlm_tokens < max_mlm_tokens:
                sample_len = np.random.choice(range(1, 11), p=mlm_length_distribution)
                ...
                if rng.random() < 0.8:
                    ...
                    if rng.random() < 0.5:
                        inputs[i][sample_pos: sample_pos + sample_len] = torch.randint(len(self.tokenizer),
...
def main():
    ...
    if model_args.config_name:
        ...
        if 'wwm' in model_args.model_name_or_path:
