...
class ElectraCombineModel(nn.Module):
    ...
    def __init__(self, discriminator, loss_weights=50.0, tokenizer=None, mlm_probability=0.15,
        ...
        self.discriminator.albert.pooler = BertPooler(discriminator.config)
    ...
    def forward(
        ...
        d_inputs_1, d_labels_1, d_inputs_2, d_labels_2, d_inputs_3 = self.mask_tokens((input_ids, start_ngram, end_ngram, start_ngram, end_ngram))
        ...
        embedding_1 = self.discriminator.albert.pooler(d_out_1.hidden_states[-1])
        embedding_2 = self.discriminator.albert.pooler(d_out_2.hidden_states[-1])
        embedding_3 = self.discriminator.albert.pooler(d_out_3.hidden_states[-1])
...
def main():
    ...
    tokenizer = AlbertTokenizer.from_pretrained(model_args.model_name_or_path, cache_dir=model_args.cache_dir)


    model_bert = AlbertForMaskedLM.from_pretrained(
